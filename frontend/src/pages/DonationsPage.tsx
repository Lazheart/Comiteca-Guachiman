...
import type { DonationRecord, PaginationParams } from '@/interfaces';
...
/**
 * Página de donaciones con listado paginado y estadísticas.
 */
export function DonationsPage() {
  const { items, loading, error, currentPage, totalPages, totalItems, setPage, refetch } =
    useServerPagination<DonationRecord>({
      fetcher: (pagination: PaginationParams, signal) => donationService.getAll(pagination, signal),
      initialPageSize: 20,
    });

  const { data: statsRows } = useApi(() => donationService.getStatistics(), []);
    ...
    <div className="section-container py-10">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          {!loading && (
            <span className="text-xs text-[#6b6b6b]">{totalItems.toLocaleString()} registros</span>
          )}
        </div>
